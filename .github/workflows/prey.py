...
print("[CLIENT] Démarrage de la proie...")
...
sem.acquire()
try:
    # On écrit l'octet correspondant à une PROIE à la position donnée
...
try:
        ...
        try:
            ...
            if moves:
                ...
                if val == bytes([c.EMPTY]):
                    ...
                    shm.write(bytes([c.PREY]), offset=target)
    ...
    try:
        ...
        shm.write(bytes([c.EMPTY]), offset=my_pos)
        sem.release()
        print("[CLIENT] Proie retirée de la carte.")
    except:
        pass